                                ...
                                <template v-if="activeCidArr.includes(Number(character.cid))">
                                    <div class="full-width column q-gutter-xs">
                                        ...
                                        <template v-for="state in character['stateArr']">
                                            <div class="full-width">
                                                <checkboxInputElement :label="state['characterstatename']" :value="selectedCsidArr.includes(Number(state['csid'])) ? '1' : '0'" @update:value="(value) => processCharacterStateSelectionChange(state, value)"></checkboxInputElement>
                                            </div>
                                        </template>
                                    </div>
                                </template>
